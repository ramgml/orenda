...
- [~] **32.1** — в работе: MiniMax-M3, branch `phase-32-1-release-v020`, с 2026-08-17. **Релиз v0.2.0** *(блокируется явной командой пользователя на push)*. CHANGELOG: секция `## [0.2.0] — <дата>` из `Unreleased` + дописать Phase 31 (31.1–31.11) и проверить sweep'ом, что каждая фаза 28.21–31 имеет запись. `VERSION` → `0.2.0`. PR `dev` → `main` по шаблону, мердж, `git tag v0.2.0` на main. CI на main зелёный (full lint gate, 30.1).
