...
- [~] **31.2** — в работе: MiniMax-M3, branch `phase-31-2-study-domain`, с 2026-08-17. Domain: `course.Course.PaceNotesMD` (+ Validate: длина разумная, trim); `task.Task.StudyCourseID` (`json:",omitempty"` — обратная совместимость по конвенции 27.3); новый пакет `internal/domain/study` — entity `Proposal`, `Validate()` (title и target_date обязательны; target_date парсится как `YYYY-MM-DD`), sentinels `ErrNotFound`/`ErrInvalidInput`/`ErrTransition` (accept/dismiss только из `pending`). Unit-тесты Validate по конвенции домена.
