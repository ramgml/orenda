...
=======
- [x] **30.10** — **закрыта 2026-08-16 в `phase-30-10-quickcapture-due`.** Бэкенд `POST /api/v1/inbox/tasks` ранее принимал `due_at` (`taskInput.DueAt`); клиент `createInboxTask` дополнен `due_at?: string`. Модалка получила опциональный `<input type="date" data-testid="quick-capture-due">`; при submit пустое поле не включается в payload (hotkey-флоу `q → Cmd+Enter` не замедлен), непустое → `new Date(${dueDate}T00:00:00).toISOString()` (local-midnight → UTC ISO, без TZ-surprise). Существующие 11 тестов `QuickCapture.test.tsx` зелёные. **Verify:** vitest 274/274; `make build`; `make test-e2e` 18/18; `golangci-lint run --new-from-merge-base=origin/dev` exit 0.
>>>>>>> phase-30-10-quickcapture-due
