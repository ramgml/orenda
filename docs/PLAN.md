...
- [ ] **30.17** **30.7 gap: `writeError` не маппит `taskservice.ErrInvalidInput` → 500 вместо 400.** Phase 30.7 (merge `bf44006`) добавила service-валидацию «reject требует comment» и записала «400 на API уровне через `writeError`», но `internal/api/errors.go` маппит только domain-сентинелы (`task.ErrInvalidInput` и др.) — не `taskservice.ErrInvalidInput`. **Подтверждено исполнением 2026-08-16 (при реализации 30.10):** `POST /api/v1/tasks/{id}/review {decision:"reject", comment:""}` → `500 {"error":"internal"}`; API-теста на приёмку 30.7 нет (только service-тест). Побочно: bogus `decision` тоже 500. Приёмка: `writeError` маппит `taskservice.ErrInvalidInput` → 400 `invalid_input`; API-тест reject без comment → 400; bogus decision → 400.
