...
>
> **2026-08-16 (как читать этот файл):** исполняемый бэклог — только **Phase 29** (постановка) и **Phase 30** (реестр: приоритеты + порядок волн в шапке). Все `[ ]` в фазах ≤ 28.x — исторические записи: реальный статус каждой фазы зафиксирован в её audit-заголовке, а невыполненные пункты оттуда перенесены в Phase 30 с номерами. Линейное чтение файла сверху вниз как очереди задач некорректно.
...
> - **Wave 1 — процессная целостность (мелкие backend):** 30.2, 30.7.
...
- [ ] **30.3** **Phase 10: VK Long Poll.** Сейчас VK только Callback API; Long Poll не реализован. Большая подфаза (транспорт + reconnect + тесты).
- [ ] **30.4** **Phase 10: Email HTML-шаблоны.** Сейчас plain text; DoD Phase 10 требовал HTML.
- [ ] **30.5** **Phase 10: Weekly digest.** В DoD Phase 10, не реализован: периодическая сводка (scheduler + агрегация + шаблон).
- [ ] **30.6** **Phase 5: `[[` autocomplete в редакторе wiki.** Планировался в 5.5, не вошёл в DoD, потерян при приёмке (аудит 2026-08-12). Tiptap suggestion по `WikiService.Tree`/поиску slug'ов; вставка `[[slug]]`. С Phase 29 страницами управляют и агенты — автокомплит должен видеть и agent-created страницы (тот же стор, бесплатно). Приёмка: ввод `[[` → dropdown с фильтрацией → выбор вставляет рабочую ссылку; vitest на suggestion-фильтр.
- [ ] **30.7** **Phase 19: backend принуждает comment при reject.** `reviewTaskHandler` (`handlers_phase3.go:216`) пробрасывает пустой `req.Comment` без валидации; фронт подсказывает, но пустая строка проходит — агент получает возврат без причины. `TaskService.Review`: reject + пустой comment → `ErrInvalidInput` (approve без комментария легален). Приёмка: API-тест reject без comment → 400; фронт маппит на существующую подсказку.
- [ ] **30.8** **Phase 4: задачи с `due_at` в календаре.** `handlers_calendar.go` отдаёт только timed-события (start_at/end_at); `CalendarPage` задачи не подгружает (grep `due` пуст); установить due_at из UI сейчас нельзя вообще (поле API/agent-only, отображение — Today-бакеты, канбан-бейдж, сайдбар). **Формат решён (пользователь, 2026-08-16): all-day маркер дедлайна** на дату due_at — стилистика канбан-бейджа (красный при overdue), клик → открытие задачи. Семантика не смешивается: start_at/end_at — timed-события, due_at — дедлайн-маркер. Источник: оговорка аудита 2026-08-12.

### P3

- [ ] **30.9** **Phase 7: backup UX — `Status`/`TestConnection` + расписание.** Git-доступ — голый `exec.CommandContext("git", …)` в `internal/backup/backup.go`; кнопки «проверить remote» в настройках нет. Snapshot идёт по тикеру 24h, не cron 03:00; `BACKUP__SNAPSHOT_ROTATION_DAYS` и schedule не UI-editable (за скобкой 28.1). Приёмка: `POST /backups/test` (есть для push — расширить статусом remote) + честная конфигурация расписания.
- [ ] **30.10** **Phase 21: due-поле в QuickCapture.** В 21.1 планировалось «опционально due»; в модалке только title (`QuickCapture.tsx`, один input). Приёмка: due сохраняется в задачу; хоткей-флоу не замедлен (поле опционально, таб-навигация).
- [ ] **30.11** **Phase 23: WIP-фидбек.** Badge лимита в шапке колонки есть; нет toast при заблокированном move (422 `ErrColumnFull` молча revert'ит optimistic move) и подсветки переполненной колонки. Приёмка: move в полную колонку → toast «N из M»; колонка на лимите визуально отмечена.
- [ ] **30.12** **Phase 17: бейджи времени на карточке.** estimate/spent/активный таймер на `TaskCard` (аудит 2026-08-14, «остаются бейджи времени»).
- [ ] **30.13** **Phase 27.6 за скобкой: структурная правка active-курса.** Granular CRUD curriculum со стабильными ID и сохранением прогресса студента; drag&drop reorder модулей/уроков; импорт программы из markdown. Актуальность растёт после Phase 29 (агенты создают курсы — правки без потери прогресса нужны чаще).
- [ ] **30.14** **Phase 27.7/27.8 за скобкой: UI статусов проекта + bulk-edit.** Добавление/переименование статус-колонок с выбором machine key; массовые операции над задачами (status/assignee/priority).
- [ ] **30.15** **Ops-гигиена скриптов.** `uninstall.sh` молча глотает неизвестные флаги, нет `--help`; `update-dogfood.sh` хардкодит remote `origin`, нет `--force`. Мелко, но режет при ops-инцидентах.
- [ ] **30.16** **Lint-остаток 73 issues (после Phase 30.1 отключили `hugeParam`).** 22 gocritic (dupImport, unnamedResult, paramTypeCombine, sprintfQuotedString, httpNoBody, rangeValCopy, equalFold), 17 unparam, 8 nilnil, 6 unused, 5 contextcheck, 3 prealloc, 3 nilerr, 3 gosimple, 2 unconvert, 2 staticcheck, 1 goimports, 1 bodyclose. Механические фиксы с diminishing returns; закрывать партиями при касании файлов либо одним свипом с обоснованием каждой группы. CI их не блокирует (PR gate — только новый код), но `make lint` локально их показывает.
