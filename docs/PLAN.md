...
- [~] **31.10** — в работе: MiniMax-M3, branch `phase-31-10-specs`, с 2026-08-17. Спеки + skill: `internal/api/openapi.yaml` + `docs/openapi.yaml` синхронно — новые пути (`/agent/study-proposals`, `/study-proposals*`, PATCH `/agent/courses/{id}`) и схемы (Proposal, enrich course payload, todayResponse.proposals); `TestOpenAPI_RouteCoverage_FullRouter` зелёный. SKILL.md: секция «Plan my day» — цикл харнесс-агента (`orenda_courses_list` → pace_notes + прогресс → `orenda_study_propose` × N → пользователь подтверждает в Dashboard), с curl-примерами по образцу §4.4. SESSION.md обновлён.
