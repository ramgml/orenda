...
- [ ] **30.1** **CI.** Локальные гейты (`make test`, `make lint`, `make test-e2e`) не enforced — регрессии заходят в `dev` незамеченными (28.21 зафиксировал как «кандидат на отдельную фазу»). GitHub Actions workflow: build + `go test ./...` + vitest + golangci-lint на PR в `dev`; E2E — отдельный job (Playwright image). Приёмка: PR с красным тестом блокирует merge; бейдж в README.
- [ ] **30.2** **`sync_ops` record-failures молчат.** `_ = syncOpsRecord(...)` ×4 в `internal/api/handlers_sync.go` (строки 146/204/226/254) — ошибка записи в outbox-таблицу теряется, sync-клиент и сервер расходятся молча. Минимум: `log.Warn` с op-id; лучше — счётчик в `/api/v1/stats` или healthz-флаг. Приёмка: инъекция ошибки записи → видимый сигнал в логе.
...
- [ ] **30.16** **Lint-остаток 95 issues.** 45 hugeParam non-api (per 28.15 commit), 15 unparam, 7 nilnil, 5 contextcheck. Механические фиксы с diminishing returns; закрывать партиями при касании файлов либо одним свипом с обоснованием каждой группы.
