...
- [~] **31.3** — в работе: MiniMax-M3, branch `phase-31-3-study-storage`, с 2026-08-17. Storage: `study_proposal_repo` (Create, ListPending, Get, MarkAccepted(id, taskID), MarkDismissed(id) — оба Mark\* переводят status + resolved_at атомарно); `course_repo`/`task_repo` — новые колонки в INSERT/UPDATE/Scan (пустые значения ↔ NULL маппинг по существующей конвенции nullString); `docs/DB.md` — строки таблиц + запись в таблице миграций.
