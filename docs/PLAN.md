...
>
> **2026-08-17 (как читать этот файл):** исполняемый бэклог — **Phase 32** (постановка 2026-08-17: dogfood migration — управление проектом внутри Orenda; после её 32.6 этот файл замораживается как архив). Phase 30 (реестр, 17 задач) и Phase 31 (11 задач) закрыты 2026-08-17; Phase 29 закрыта 2026-08-16. Новая работа до заморозки появляется двумя путями: постановка новой фазы (номер 33+) или новая нумерованная задача в реестре Phase 30. Все `[ ]` в фазах ≤ 28.x — исторические записи: реальный статус каждой фазы зафиксирован в её audit-заголовке. Линейное чтение файла сверху вниз как очереди задач некорректно.
...
| Было (git) | Станет (Orenda) |
|---|---|
| Задачи реестра 30.x / фаз | Задачи в проекте «Orenda dev»; claim — нативный `POST /agent/tasks/{id}/claim` (lock + 409 с holder вместо гонок по маркерам `[~]` в git) |
...
- Dogfood-бинарь (`main` @ `5dcadb3`, 2026-08-16) отстаёт от `dev` на **126 коммитов**: нет 28.21 (login rate-limit — security), 28.22–28.23, Phase 29 (agent surfaces), всего реестра 30, всей Phase 31. `/api/v1/info` на инстансе: `websocket:false, fts:false, backup:false, bots:false`.
- **Backup инстанса не настроен**: `backup_settings` пусто, `snapshots/` пусто, `backup_log` пусто — план переезжал бы в единственный файл `orenda.db` без копий. Механика готова: git push mirror-директории на `backup.remote_url` (`internal/backup/backup.go:50`), hot-reload настроек (28.9), restore endpoint с verify (`POST /api/v1/backups/restore`).
- Релизная механика: `VERSION` (сейчас `0.1.0`) — source of truth; тег на main `vX.Y.Z`; pre-1.0 minor bump ⇒ **v0.2.0**. CHANGELOG `Unreleased` содержит 28.21–28.23 + 29 + 30.x, **не содержит Phase 31** (11 задач — дописать при релизе).
- Push в remote/main и создание внешнего репозитория — только по явной команде пользователя (правило репозитория); задачи 32.1–32.3 содержат такие шаги и блокируются командой.
- Restore-дрилл: CLI `backup restore` с verify (Phase 22) восстанавливает в указанный каталог — дрилл не трогает живую БД.

**Задачи:**

- [ ] **32.1** **Релиз v0.2.0** *(блокируется явной командой пользователя на push)*. CHANGELOG: секция `## [0.2.0] — <дата>` из `Unreleased` + дописать Phase 31 (31.1–31.11) и проверить sweep'ом, что каждая фаза 28.21–31 имеет запись. `VERSION` → `0.2.0`. PR `dev` → `main` по шаблону, мердж, `git tag v0.2.0` на main. CI на main зелёный (full lint gate, 30.1).
- [ ] **32.2** **Обновление dogfood.** `scripts/update-dogfood.sh` (guard main+clean сработает после 32.1). Smoke после рестарта: `/api/v1/info` версия `v0.2.0`; capabilities websocket/fts — true; login работает; WS-handshake 101; SPA отдаётся из embed; миграция 022 применена (`pace_notes_md`, `study_course_id`, `study_proposals` существуют); `make build`-артефакт не задевает dev-порт 2138.
- [ ] **32.3** **Backup + restore-дрилл** *(блокируется созданием remote — пользователь или по его команде `gh repo create`)*. Приватный репозиторий `orenda-backup`; `backup.remote_url` в `~/.local/share/orenda/config.yaml` (hot-reload, рестарт не нужен); первый `POST /api/v1/backups/snapshot` + push; `GET /api/v1/backups/status` показывает снапшот. Дрилл: CLI restore-with-verify в tmp-каталог → восстановленная БД открывается, `SELECT COUNT(*)` по tasks/agents совпадает с живой. Зафиксировать RPO (интервал снапшотов) в `docs/ARCHITECTURE.md`.
- [ ] **32.4** **Документ конвенции + входная точка агентов.** `docs/DOGFOOD.md`: таблица переноса (выше), workflow задачи (постановка в wiki → задача со ссылкой → claim → PR → review в Orenda → merge), правило «новая работа = задача в инстансе, не строка в PLAN.md» (заменяет правило реестра Phase 30). `AGENTS.md`: входная точка меняется на `orenda agent next` + MCP; SESSION.md из «читай первым» убирается. `opencode.json` `instructions`: SESSION.md/PLAN.md → `docs/DOGFOOD.md` (+ AGENTS.md остаётся). Аналогично глобальные инструкции других харнессов, если ссылаются на SESSION.md.
- [ ] **32.5** **Пилот: 3 реальные задачи полным циклом в инстансе.** Кандидаты (принести постановки в wiki при старте): курсовой activity-контур (долг из 29.5, номера не получил), generic `POST /agent/tasks`, nightly-триггер планирования (если пользователь передумает) — или свежий бэклог на момент пилота. Каждая: постановка в wiki → задача → claim агентом → PR → review (approve/reject в UI) → merge. Трение фиксируется комментариями в задаче пилота; найденные продуктовые gaps → новыми задачами в инстансе (не в PLAN.md).
- [ ] **32.6** **Заморозка файлового бэклога.** Шапка PLAN.md и SESSION.md: пометка «заморожено <дата>, актуальный бэклог — инстанс 2137, проект „Orenda dev“». Открытые «за скобкой» всех фаз либо получают задачи в инстансе, либо явно закрываются как «не делаем». Финальная сверка: ни один источник не ссылается на PLAN.md как на очередь.

**DoD (проверяется исполнением):** dogfood на v0.2.0 с websocket/fts; backup push зелёный + restore-дрилл пройден; 3 пилотные задачи прошли claim→review→merge без единой правки PLAN.md ради координации; агент, начинающий сессию, получает работу из `orenda agent next`, а не из файлов; PLAN.md/SESSION.md несут морозильную пометку.

**За скобкой (зафиксировано):** multi-user / multi-device sync — следующая эра, постановкой не покрывается; перенос истории фаз из PLAN.md в wiki не делаем (архив остаётся в git); автоматическая синхронизация «задача ↔ PR» (webhook GitHub → комментарий в задаче) — отдельная задача при потребности.
