...
> - **Wave 3 — средние:** 30.6, 30.9, 30.14; 30.8 — только после продуктового решения о формате (all-day entry vs точка на дате) — вопрос вынесен в задачу, решает пользователь.
...
- [ ] **30.8** **Phase 4: задачи с `due_at` в календаре.** `handlers_calendar.go` отдаёт только timed-события; `CalendarPage` задачи не подгружает (grep `due` пуст). Решение формата (all-day entry vs точка на дате) — часть задачи. Источник: оговорка аудита 2026-08-12.
