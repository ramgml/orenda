...
make test           go test ./... -race -count=1  (full, uncached — CI backstop / release gate)
                    npx vitest run

make test-gate      same suites without -count=1 (Go test cache enabled)
                    — the local pre-push gate; seconds on an unchanged tree
