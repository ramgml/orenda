...
Канонические прецеденты в истории репо: полный release-цикл — `release-v0.14.1`
→ merge в `promotion-v0.14.1` → merge в `main` (`v0.14.1`) с обратным мержом в
`dev` (merge-коммит `bf626eb`/`3e33309`); свежий канонический релиз —
`release-v0.15.0` → PR #143 → `main` → тег `v0.15.0`.
...
| ~~Promotion~~ (устарело) | — | `main` мимо `dev` | ~~`promotion-vX.Y.Z`~~ | — | не выполнялся — в этом и был вред | запрещено, см. [«Устаревшие практики»](#устаревшие-практики) |
...
4. **Обратный мерж — обязателен.** `main` (с новым тегом и метаданными) мержится
   ...
   стартует от устаревшей базы (прецеденты: `3e33309` после v0.2.0, обратный
   мерж после v0.14.1 — `bf626eb` в истории `dev`).
...
**Promotion-PR в `main` мимо `dev` (`promotion-v0.14.1`, `promotion-v0.14.2`) —
устарело и запрещено.**

- **Что было:** в эпоху v0.14.x код для релиза собирали в отдельных
  `promotion-*` ветках и мержили их прямо в `main` (PR #137, PR #141), без
  релизной ветки и без обратного мержа в `dev`.
- **Почему сломалось:** фиксы из той истории так и не доехали до `dev` — тег
  `v0.14.2` **не является предком** `dev`
  (`git merge-base --is-ancestor v0.14.2 origin/dev` → `false`); integration
  trunk и production разошлись. Сами ветки при этом остались висеть в remote
  как неубранный мусор.
- **Что теперь:** релизы — по release-циклу выше (`release-vX.Y.Z` с
  обязательным обратным мержем; канонический пример — `release-v0.15.0` →
  PR #143 → `v0.15.0`). Не резюмецировать `promotion-*` ни для новых релизов,
  ни для хотфиксов.
