...
5. **PR** — по шаблону `.github/PULL_REQUEST_TEMPLATE.md`. Merge в `dev` выполняет **PM (omp)** по протоколу шага 6 — агент свой PR не мержит. После merge: `git worktree remove` + prune ветки (делает PM); локальный `dev` догоняется до `origin/dev` только fast-forward (ритуал — AGENTS.md «Worktree per task»).
6. **Review в Orenda** — `orenda agent submit <id>`. С 2026-08-20 ревью и мерж PR делает **PM (omp)**: DoD по доказательствам (прогоны/выводы, не «implemented»), чтение диффа целиком (скоуп, конвенции, аддитивные миграции, без стабов), mergeable-проверка против свежего `origin/dev`; рискованные диффы (auth/security/миграции, >~400 строк) — дополнительно через reviewer-сабагента. Неполный DoD или сомнение в качестве — возврат агенту с конкретикой, без мержа. Карточку на `/review` закрывает владелец (у агентского токена нет review-прав) — это его **аудит-лог мержей PM**; возврат там = откат через новую задачу, всегда с комментарием — это обратная связь, а не сбой.
