...
он в `.gitignore`, коммитить его нельзя**. Guard пер-репо: в чужом
чекауте без правила `.orenda/` в `.gitignore` `git add .` утащит токен —
добавьте правило до использования локального конфига. Сломанный (непарсящийся)
