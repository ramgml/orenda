...
> Правила кода — в `AGENTS.md`; концепции домена — в `docs/context/CONTEXT.md`;
> процедуры agent API — в `../skills/orenda/SKILL.md`.
...
Доступ настраивается один раз на хосте. Цепочка приоритета CLI — по
каждому полю независимо: флаги (`--url`/`--token`) > env (`ORENDA_URL` /
`ORENDA_AGENT_TOKEN`) > `./.orenda/agent.yaml` в корне проекта
(алиас `.yml` читается так же; при обоих файлах побеждает `.yaml` —
T242) > `~/.config/orenda/agent.yaml` (`url` + `token`, chmod 600). Проектный
`.orenda/agent.yaml` (T178) позволяет чекауту нести свои настройки
подключения: он читается из текущего рабочего каталога и **держит секрет —
он в `.gitignore`, коммитить его нельзя**. Guard пер-репо теперь в самом CLI
(T181): при резолве он предупреждает в stderr — файл уже tracked by git (токен
уйдёт в историю: `git rm --cached` + ротация токена), файл не покрыт `.gitignore`
(`git add .` закоммитит секрет — добавьте `.orenda/`), права шире 0600 (`chmod 600`);
в чужом чекауте warning появится при первом же вызове, вне git-репозитория CLI
молчит. Сломанный (непарсящийся)
локальный файл — явная ошибка с путём, а не молчаливый провал к глобальному
конфигу. Глобальный файл на основной рабочей машине уже создан:
**`orenda agent me` работает без флагов, токен искать не нужно**. Если на
новом хосте файла нет — токен берётся из MCP-конфига harness'а
(`~/.codex/config.toml`, сервер `orenda`) или минтится новым агентом через
UI; plaintext токена сервер не хранит, перепоиск по файловой системе —
признак того, что файл забыли создать. Диагностика: `orenda agent config`
печатает resolved url/token и источник каждого поля (flag/env/local/
global); токен по умолчанию маскируется, сырое значение — `--json
--show-secret`. `mcp-proxy` ходит по той же цепочке: при проектном конфиге
инлайн-токен в `mcp.json` больше не нужен. Смешанные источники (url из
проектного конфига, токен извне) легальны, но токен уйдёт на этот url — CLI
предупреждает в stderr, а `orenda agent config` показывает источник каждого
поля.
...
orenda agent next        # КЛЕЙМИТ первую готовую задачу (mutation!); --peek = только посмотреть; exit 2 = работы нет; печатает #N рядом с UUID
