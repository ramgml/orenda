...
3. **Финиш** — merge `--no-ff` в `main` через PR (прецедент: `release-v0.15.0`
   ...
   `main`.
4. **GitHub Release — автоматически, пушем тега.** CI-джоба `release`
   (`.github/workflows/ci.yml`, Task 346) после зелёного гейта (lint/test/
   build/e2e) вырезает секцию `## [X.Y.Z]` из `CHANGELOG.md` и публикует
   релиз (`--latest`, идемпотентно: существующий релиз обновляется).
   Создавать релизы руками (`gh release create`) больше **не нужно**; ручной
   шаг остаётся только если нужно приложить бинарные ассеты. Пустая секция
   changelog валит джобу — метаданные релиза не должны забываться в п.2.
5. **Обратный мерж — обязателен.** `main` (с новым тегом и метаданными) мержится
...
6. **Ветка удаляется** (`git push origin --delete release-vX.Y.Z` +
...
`make lint-new BASE_REF=origin/main` + `make test-full`, тег (GitHub Release публикует CI-джоба `release`),
