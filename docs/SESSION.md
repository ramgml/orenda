...
- **docs/CONTEXT.md (2026-08-13).** Заведён файл концепций: семантика домена (канбан=статусы, LMS-цикл, контур делегирования, inbox, две auth-модели, бэкап/restore) — пробел между PRD (vision) и PLAN/API (задачи/эндпоинты). Хартия против протухания: только смерженное в dev, без статусов/дат в теле секций, правка секции в том же PR что и смена концепции. Зарегистрирован в AGENTS.md «Key files to read first».
