...
func TestParseCourseRef(t *testing.T) {
	cases := []struct {
		...
		{"C1", "C1", 1, true},
		{"C999", "C999", 999, true},
		{"C1000000", "C1000000", 1000000, true},
...
func TestCourseRefNotFoundError(t *testing.T) {
	err := &RefNotFoundError{Ref: "C7"}
	assert.Equal(t, "course C7 not found", err.Error())
	assert.ErrorIs(t, err, ErrNotFound)

	err2 := &RefNotFoundError{Ref: "c999"}
	assert.Equal(t, "course c999 not found", err2.Error())
...
func TestParseLessonRef(t *testing.T) {
	cases := []struct {
		...
		{"L1", "L1", 1, true},
		{"L999", "L999", 999, true},
		{"L1000000", "L1000000", 1000000, true},
...
func TestLessonRefNotFoundError(t *testing.T) {
	err := &LessonRefNotFoundError{Ref: "L10"}
	assert.Equal(t, "lesson L10 not found", err.Error())
	assert.ErrorIs(t, err, ErrNotFound)

	err2 := &LessonRefNotFoundError{Ref: "l42"}
	assert.Equal(t, "lesson l42 not found", err2.Error())
	assert.ErrorIs(t, err2, ErrNotFound)
}
