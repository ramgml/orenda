...
+ env + flags — a real code change can never be served stale). This is
the everyday developer test and the local pre-push gate. The full
uncached `make test-full` (`-count=1`) is the contract for the CI
...
- Deprecated and forbidden: «promotion» PRs straight into `main` past `dev` with no back-merge (the `promotion-v0.14.1` / `promotion-v0.14.2` practice). Concrete damage: those fixes never reached `dev` — `v0.14.2` is not an ancestor of `dev` (`git merge-base --is-ancestor v0.14.2 origin/dev` → false). Never resurrect this pattern.
