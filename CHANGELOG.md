...
- **Task 49 (PR #61):** W<N> ref format for wiki pages. Wiki pages now carry a W-prefixed reference (`W7`) in REST paths, MCP `slug` arguments, and a copyable `WikiNumberChip` in the UI. Migration `037` adds the `wiki_page_number` column and sequence. Slugs matching the pattern `W<digits>` are rejected with 422 `invalid_slug_reserved`.
- **Task 50 (PR #60):** P<N> ref format for projects. Projects now carry a P-prefixed reference (`P3`) in REST paths, CLI args, MCP `project_id` arguments, and a copyable `ProjectNumberChip` in the UI. Migration `036` adds the `project_number` column and sequence.
- **Task 51 (PR #62):** C<N> and L<N> ref formats for courses and lessons. Courses carry a C-prefixed reference (`C5`), lessons carry a globally-numbered L-prefixed reference (`L12`) across all courses. Migrations `038`/`039` add the `course_number`/`lesson_number` columns and sequences. Copyable chips in the UI.
