...
- **Phase 30.10:** QuickCapture modal gains an optional due-date input (`<input type="date" data-testid="quick-capture-due">`). The field is opt-in — leaving it empty keeps the hotkey flow (`q → Cmd+Enter`) one keystroke from thinking to saved. When set, the value is converted to local-midnight ISO 8601 and sent as `due_at`. The server-side `POST /api/v1/inbox/tasks` already accepted `due_at`, so no backend changes were required; only the client type and modal render.
