...
### Added
- **Task 356 (PR #252):** Spent-time fallback for legacy tasks — tasks that predate the Task 87 auto-timer (no `time_entries` rows, zero stored `time_spent_s`) now surface their `in_progress` history on every read surface: the derivation walks the `task.status_changed` audit timeline (pure functions in `internal/domain/timeentry/status_spent.go`, batched via `StatusChangesByTasks` + `HasAnyEntriesByTasks` — no N+1), and task reads (single GET, project/inbox listings, `GET /reports/time` with window clipping) stamp the value virtually without persisting it. A stored counter >0 or any entry disables the fallback per task. Pinned by `status_spent_test.go`, `handlers_spent_fallback_test.go`, `report_fallback_test.go`; documented in `docs/context/API.md`.
- **Task 354 (PR #251):** time report by project — `GET /reports/time` tags every task row with `project_id`/`project_name`/`project_color` and gains a `projects[]` array of per-project subtotals (sorted by `total_sec` desc; projectless tasks keep empty project fields and never enter `projects[]`); backed by `task.Repository.InfosByIDs` — one batched tasks→projects LEFT JOIN returning title + project id/name/colour (replaces the report's title-only `TitlesByIDs` lookup, its only caller; the event-reminder path keeps `TitlesByIDs`); openapi (canonical + embedded copy) documents the new `TimeReport`/`TimeReportTask` fields; ReportsPage renders one section per project — coloured dot + name + subtotal header, task bars tinted in the project colour (`#94a3b8` fallback when empty), sections sorted by subtotal desc, trailing «No project» section for projectless tasks. No migration — `projects.color` already shipped.

### Changed
- **Task 352 (PR #249):** `make web-test` accepts a `TARGETS` path filter — `make web-test TARGETS=src/features/tasks` scopes the vitest run to one feature directory instead of guessing raw `npx vitest` invocations (symmetric with `web-typecheck` for single-feature checks); documented in AGENTS.md.
- **Task 346 (PR #247):** CI auto-publishes the GitHub Release — pushing a `vX.Y.Z` tag now runs a `release` job (`.github/workflows/ci.yml`, after the full lint/test/build/e2e gate) that cuts the matching `## [X.Y.Z]` section out of CHANGELOG.md and creates the release (`--latest`; idempotent — an existing release gets its notes updated). An empty/missing changelog section fails the job instead of publishing blank notes. Manual `gh release create` — the practice that silently lapsed over v0.22.0–v0.23.1 and was caught up by hand on 2026-09-20 — is no longer needed; GITFLOW.md release flow updated.

### Fixed
- **Task 351 (PR #250):** SPA deploys reach users — the generated service worker was registered by the vite-plugin-pwa silent `registerSW.js` inject, which never drove the update lifecycle: after a deploy the new precache sat in an installing/waiting worker while the page kept running the old controller's bundle (only manual unregister + `caches.delete` helped). Registration now goes through `virtual:pwa-register` from app code (`web/src/shared/pwa/registerServiceWorker.ts`): with `registerType: 'autoUpdate'` the helper sends SKIP_WAITING to a waiting update and reloads the page once when the new worker activates — deploy → next reload → new bundle. No-op on unsupported browsers and insecure contexts; registration errors are logged and swallowed so a broken SW never breaks app boot; the wiring is pinned by a 114-line test.
- **Task 348 (PR #248):** task-sidebar time tracking fixed — `TimerLauncher` (the pub-sub that arms `StartTimer()`) is mounted in the idle state again: since Task 95 the widget rendered nothing while idle, so the sidebar's «Start timer» button was a silent no-op exactly when a timer needed starting (regression test drives `StartTimer` in the idle state and asserts the API call fires; verified in headless Chrome against the built bundle). The EstimateEditor row also fits the ~246px sidebar now: estimate input `h-7 min-w-0 flex-1`, Set/clear buttons `h-7 shrink-0 px-2` — uniform height, no clipping past the card edge (utilities pinned in jsdom; real hit-check at 1152px/768px in the PR evidence).
...
### Added
- **Task 178 (PR #191):** per-field agent config resolution — every `orenda agent` / `mcp-proxy` setting resolves through the chain `flag > env > ./.orenda/agent.yaml > global config`, so a repo-local `./.orenda/agent.yaml` now points sibling tools at the right instance without env plumbing; new `orenda agent config` prints the resolved values with their source (flag/env/global), the token masked to 4 runes, and a stderr warning when values come from mixed sources; a broken YAML file is a loud error instead of silent fallback. Provenance and the chain pinned by table-driven tests; `docs/context/DOGFOOD.md` documents the layout and diagnostics.
- **Task 188 (PR #200):** `orenda agent next` is peek-safe — plain `next` claims the first ready task (documented loudly as a mutation), the new `--peek` flag prints the ready queue without claiming, and `--await N` long-polls (chunks ≤60s) and claims as soon as work appears, exiting 2 when the budget is exhausted; invalid combinations (`--peek --await`, `--group-by --await`) are explicit errors. `orenda agent await` now subscribes to the `tasks` topic, so long-polls wake for human-created tasks too. SKILL.md and DOGFOOD.md rewritten around the claim-vs-peek contract.
- **Task 185 (PR #197):** docs and CLI help advertise the T-ref form (`T42`) as the only task reference; legacy `#N`/bare-`N` inputs now get an explicit migration hint instead of a bare lookup failure.
- **Task 189 (PR #201):** OpenAPI documents the second cause of a 404 on `POST /agents/{id}/regenerate-token` — a dangling `agents.token_id` (token row deleted) is indistinguishable from an unknown agent.

### Changed
- **Task 192 (PR #203):** human task creation now broadcasts `task.created` — `POST /projects/{id}/tasks`, `POST /inbox/tasks` and the sync `create_task` op publish the same websocket event agent proposes always did (without the propose-only `actor` field; sync replays of an already-applied `client_id` emit nothing), so open kanban boards refetch and `orenda agent next --await` long-polls wake for human-created tasks.
- **Task 175 (PR #193):** token rotation re-stamps `expires_at` — `RotateToken` previously replaced only the bcrypt hash, keeping the original mint's expiry (or a stale deadline); rotation and registration now share one expiry policy (`Service.TokenTTL`) applied in a single atomic `UPDATE` of hash + `expires_at`. Default TTL stays `0` (tokens never expire), so existing installs see no behavior change.
- **Task 181 (PR #196):** the agent-config guard now warns when a project-local agent config is tracked by git or not gitignored (tracked-state checked before the ignore rules), catching credentials that would be committed or silently lost on clone.
- **PR #189:** back-merge of `release-v0.18.0` into `dev` after the v0.18.0 release — the mandatory step from `docs/context/GITFLOW.md` «Release-цикл» was skipped for every release since v0.2.0; this folds the VERSION/CHANGELOG release-prep commits back so `dev` carries them.
- **Tooling (Task 179, PR #202):** the Phase 30.16 lint-debt sweep — ~35 behavior-preserving extraction refactors across `internal/api`, `service`, `storage`, `bot`, `backup`, `cmd` and `mcp` (field-group/audit/scope/step helpers, per-op-kind sync dispatch, per-group MCP registrars) to satisfy gocognit/gocritic; no wire-format changes.
- **Tooling (Task 180, PR #204):** knip triage landed as a hard gate — dead e2e/offline helpers dropped, intra-file types unexported, and `make web-knip` is now blocking in the pre-push hook (allow-list limited to regenerable shadcn/ui primitives).
- **Tooling (Task 191, PR #205):** `.golangci.yml` sets an explicit `gocognit` min-complexity 30 post-T179 (tightening to 25 deferred — 8 findings at 25).
- **Tooling (Task 183, PR #195):** the golangci-lint ≥ v2 requirement (`.golangci.yml` carries `version: "2"`) documented in the local-gates docs.
- **Tooling (Task 187, PR #199):** the release-gate GitHub Actions workflow upgraded to golangci-lint v2, matching the local gate.
- **Tooling (Task 176, PR #194):** the e2e CI job runs in the Playwright container image instead of installing Chromium per run (no `make` in the image — playwright invoked directly; artifact binary `chmod +x` restored since `upload-artifact` drops the exec bit).

### Fixed
- **Task 193 (PR #207):** 500 on task create/PATCH and wiki save when `backup.enabled=false` — `serveBackupIfEnabled` returned a typed-nil `*mirror.Service` that satisfied the `MirrorWriter` interface, so every `Mirror == nil` guard passed and the first mirror write dereferenced the nil receiver. `runServe` now constructs the mirror unconditionally (it is a filesystem projection of the DB, not part of the backup machinery), and every mirror entry point tolerates a nil receiver (regression test covers all four).

### Security
- **Task 174 (PR #190):** same-origin CSRF check for cookie-authenticated mutations — non-safe methods are rejected with `403 csrf_origin_mismatch` when an `Origin`/`Referer` host differs from the request host (defense in depth on top of SameSite=Lax; `Origin: null` is rejected outright). Clean `Authorization: Bearer` clients (agents, CLIs — which never carry the cookie) are exempt: a cross-site page cannot plant custom headers. A request carrying BOTH the header and the session cookie is treated as a browser request and falls through to the Origin check, so a bogus Bearer cannot launder a cookie mutation (T174-CSRF-AUTHZ-SKIP-BYPASS). Header-less non-browser clients (curl, scripts) keep working.
- **Task 182 (PR #198):** expired API tokens are rejected — `verifyAPIToken` now enforces `api_tokens.expires_at`, and an expired token fails with the same generic 401 as an unknown one (wire-indistinguishable); previously an expired row kept authenticating forever. Also: a dangling `agents.token_id` during rotation now maps to 404 `token_not_found` instead of a 500 (regression-test documented pre-fix behavior).
- **Task 190 (PR #206):** constant-work `verifyAPIToken` (CWE-208, informational; found in the PR #198 security review) — token verification previously returned on the first bcrypt match, leaking the matched row's position through response time; it now always completes a full pass over every hash and answers only after it, so every request costs the same N comparisons. T182 semantics unchanged.
...
- **Docs (PR #147):** Git flow documented: hybrid main+dev with release/hotfix branches and mandatory back-merges (`docs/context/GITFLOW.md`, rewritten `AGENTS.md` «Git workflow» + «Git for AI agents»).
...
### Changed
- **Docs (workflow):** QA-гейт перед `orenda agent submit` — перевод задачи в review только после того, как PM подготовил manual QA-чеклист «Как протестировать» (новая секция в PR-шаблоне) и поднял preview-инстанс из worktree задачи (порт из 21400–21499, отдельный `data/`, тестовый юзер; docs-only диффы — без инстанса). Детали: wiki «Decision log» 2026-08-29, `docs/context/DOGFOOD.md` «QA-гейт перед submit».
...
- **Task 87 (PR #113):** Docs — `docs/context/API.md` and `docs/openapi.yaml` document the new `/agent/tasks/{id}/time` route and the 422 `time_not_logged` submit-gate response.
...
- **PR #34:** Added manual GitHub Release step to `docs/context/RELEASE.md` — the v0.4.0 release shipped without a GitHub Release (only a tag), now documented as a required step.
...
### Docs
- `docs/context/RELEASE.md` — the release process (snapshot of wiki:release-process): branch/tag model, prep → PR → tag → dogfood-update steps, and the prohibitions (no release tags on `dev`, no merge to `main` without the release gate).
- `docs/context/DOGFOOD.md` — agent entry point now documents the one-time auth setup CLI chain and the token priority order (`~/.config/orenda/agent.yaml`), so a fresh agent session no longer hunts for URL/token.
...
### Docs
- **Phase 32.4:** `docs/context/DOGFOOD.md` — the dogfood convention: project management (queue, постановки, review loop) lives in the running instance, not in PLAN.md; agent entry-point rules. `AGENTS.md` gained the local-gates (32.6) and worktree-per-task sections.
...
- **Phase 28.11:** `docs/context/ARCHITECTURE.md` (556 lines, 13 sections).
...
- **Phase 28.18:** `docs/context/PLAN.md` + `SESSION.md` sync reflecting 28.7–28.17.
