...
- **Task 149:** scheduler test perf — the snapshot cron-loop's sleep-until-next-fire is now an injectable `FireTimer` (`Scheduler.WithFireTimer`; production keeps the real `time.NewTimer` with unchanged semantics). `TestScheduler_SnapshotLoop_FiresOnCron` drives the fire deterministically (44.4s → ~0.3s of body time; the first test in a binary still pays the one-time ~1.6s template-DB build from T147). Compensator: `TestScheduler_RealTickSmoke` waits out one real cron minute tick and proves the production timer wakes up — gated behind `ORENDA_TEST_SLOW=1`, run via the new `make test-slow` target (nightly / pre-release, not part of `make test`).
