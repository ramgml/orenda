...
describe('NotificationsBell', () => {
  ...
  it('renders one entry per notification with the parsed payload', async () => {
    stubHttp.get.mockResolvedValueOnce({
      data: {
        notifications: [
          makeNotification({ id: 'n-1', payload: '{"title":"First","body":"Body 1","link":"/tasks/1"}' }),
          makeNotification({ id: 'n-2', payload: '{"title":"Second","body":"Body 2","link":"/tasks/2"}' }),
        ],
        unread: 2,
      },
    })
  ...
  it('refetches on a WS "notifications" event', async () => {
    ...
    wsClient['listeners'].get('notifications')?.forEach((fn) =>
      fn({ topic: 'notifications', body: {} }),
    )
  ...
  it('falls back to the notification type when payload has no title', async () => {
    stubHttp.get.mockResolvedValueOnce({
      data: {
        notifications: [
          makeNotification({ id: 'n-1', type: 'agent.offline', payload: '{"body":"Agent went offline"}' }),
        ],
        unread: 1,
      },
    })
