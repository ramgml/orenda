...
function makeReport(overrides: { tasks?: Array<{ task_id: string; title: string; total_sec: number }>; total_sec?: number } = {}): {
  agent_id: string
  from: string
  to: string
...
describe('ReportsPage', () => {
  ...
  it('changing From refetches the report with the updated window', async () => {
    ...
    const lastCall = stubHttp.get.mock.calls[calls.length - 1] as [string, { params: { from: string; to: string } }]
