...
describe('taskDueState', () => {
  it('returns "done" when status is done', () => {
    expect(taskDueState({ status: 'done', due_at: '2020-01-01T00:00:00Z' }, new Date('2026-08-11'))).toBe('done')
  })

  it('returns "done" when completed_at is set', () => {
    expect(taskDueState({ status: 'todo', completed_at: '2026-08-10T00:00:00Z' }, new Date('2026-08-11'))).toBe('done')
  })
  ...
  it('returns "overdue" when due_at is before today', () => {
    expect(taskDueState({ status: 'todo', due_at: '2020-01-01T00:00:00Z' }, new Date('2026-08-11T00:00:00Z'))).toBe('overdue')
  })
  ...
  it('returns "today" when due_at is the same calendar day', () => {
    ...
    expect(taskDueState({ status: 'todo', due_at: '2026-08-11T12:00:00Z' }, new Date('2026-08-11T12:00:00Z'))).toBe('today')
  })

  it('returns "upcoming" when due_at is later than today', () => {
    expect(taskDueState({ status: 'todo', due_at: '2026-09-01T00:00:00Z' }, new Date('2026-08-11T00:00:00Z'))).toBe('upcoming')
  })
...
describe('formatDueDate', () => {
  it('formats same year as "day month"', () => {
    expect(formatDueDate('2026-08-12T00:00:00Z', new Date('2026-08-11T00:00:00Z'))).toMatch(/12\s+авг/)
  })
