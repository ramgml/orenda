  ...
  return (
    <ol className="space-y-2">
      {items.map((it) => (
        <li
          key={it.id}
          className="rounded border border-border bg-white dark:bg-slate-950 px-3 py-2 text-sm flex items-center gap-3"
        >
          <span className="font-mono text-xs text-slate-400 w-32 flex-shrink-0">
            {formatRelative(it.created_at)}
          </span>
          <span className="px-1.5 py-0.5 rounded text-xs uppercase tracking-wide bg-muted text-slate-600 dark:text-slate-300">
            {it.action}
          </span>
