...
describe('TimeBadge', () => {
  it('renders nothing when nothing is tracked', () => {
    const { container } = render(
      <TimeBadge estimateS={null} spentS={0} timerActive={false} />,
    );
  ...
  it('renders spent-only when no estimate is set', () => {
    render(
      <TimeBadge estimateS={null} spentS={120} timerActive={false} />,
    );
  ...
  it('renders spent/estimate when both are set', () => {
    render(
      <TimeBadge estimateS={1800} spentS={600} timerActive={false} />,
    );
    const el = screen.getByTestId('time-badge');
  ...
  it('flips the over-budget color class when spent > estimate', () => {
    const { container } = render(
      <TimeBadge estimateS={60} spentS={120} timerActive={false} />,
    );
  ...
  it('uses H:MM:SS when hours > 0', () => {
    render(
      <TimeBadge estimateS={3700} spentS={3661} timerActive={false} />,
    );
    const el = screen.getByTestId('time-badge');
  ...
  it('renders the active-timer pulse regardless of spent/estimate', () => {
    render(
      <TimeBadge estimateS={null} spentS={0} timerActive={true} />,
    );
  ...
  it('clamps negative seconds to 0 (defensive — backend can return < 0 on race)', () => {
    render(
      <TimeBadge estimateS={null} spentS={-5} timerActive={false} />,
    );
