...
export function NewProjectInline({ collapsed = false }: NewProjectInlineProps): JSX.Element {
    ...
    return (
      <div className="px-2 py-1 flex justify-center">
        ...
        {open && (
          <form
            onSubmit={submit}
            className="absolute left-12 ml-1 z-20 flex gap-1 p-1.5 rounded border border dark:border-border bg-background dark:bg-background shadow"
          >
            <Input
              autoFocus
              type="text"
  ...
  if (!open) {
    return (
      <Button
        ...
        className="mx-2 mb-2 mt-1 w-[calc(100%-1rem)] justify-start gap-2 px-2 py-1.5 h-auto text-xs text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 border-dashed border dark:border-border"
      >
  ...
  return (
    <form
      onSubmit={submit}
      className="mx-2 mb-2 mt-1 p-2 rounded border border dark:border-border bg-background dark:bg-background"
    >
      <Input
        autoFocus
        type="text"
