...
const SuggestionList = forwardRef<SuggestionListHandle, SuggestionRenderProps>(
  function SuggestionList({ items, command }, ref) {
    ...
    if (!items.length) {
      return createPortal(
        <div
          ...
          className="rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg px-3 py-2 text-xs text-slate-500"
        >
          No matching pages.
        </div>,
        document.body,
    ...
    return createPortal(
      <div
        data-testid="wiki-link-suggestion"
        className="rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg py-1 text-sm min-w-[180px] max-w-[320px]"
        role="listbox"
      >
        {items.map((item, idx) => (
          <button
